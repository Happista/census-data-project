...
# Problem Statement:
#   
# In this project, initially we need to preprocess the data and then develop an 
# understanding of different features of the data by performing exploratory
# analysis and 
# creating visualizations.Further, 
# after having sufficient knowledge about the attributes we will perform a
...
censusData<-read.csv("censusData.csv", stringsAsFactors = F)
 View(censusData)
...
select(censusData, education)->c_ed
View(c_ed)
View(census_ed)
head(census_ed)
...
select(censusData, 5,8,11)->c_col
View(c_col)
View(census_col)
head(census_col)
...
# column.
ggplot(censusData, aes(x=relationship))+geom_bar()
...
ggplot(censusData,aes(x=relationship,fill=race))+
  geom_bar()+labs(x="Categories of Relationships",y="Count of Categories")
